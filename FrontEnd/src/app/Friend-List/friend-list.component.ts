...
export class FriendListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.loginService.getFriends(this.loginService.loggedInUser.id).subscribe(
  ...
  displayProfile(u: User): void {
    ...
    if (index > -1) this.router.navigate(['/profile', u.id]);
    else alert('User not found!');
  }

  goToChat(u: User): void {
    this.router.navigate([
      '/chat/' + u.id + '/' + this.loginService.loggedInUser.id,
    ]);
  }
}
